...
export const useCalendar = (props: { modelValue: Dayjs, disabledBtn: boolean }, context: any) => {
    const {
      setRange,
      setHoveredDate,
      isDisabled,
    ...
    const isSameMonth = (date: Dayjs) => date.isSame(props.modelValue, 'month');
    const dayClass = (date: Dayjs) => isSameMonth(date) ? getDayClass(date) : 'disabled';
